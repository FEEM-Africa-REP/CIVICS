...
Goal:
    Adding additional layer of environemtal extansions to the KENYA SAM
    using EORA Database.
    
    The idea is to have an environmental extension in the form of KENYA SAM.
"""

# Setting the directory:

# Please insert the directory in which you store the code in your computer
path = r'D:\FEEM\SUT-SAM Kenya'
import os
os.getcwd()
os.chdir(path)
os.getcwd()
...
Gi = pd.read_excel(r'Concordance Matrices\Gi.xlsx',sheet_name = 'Gi',index_col = [0],header = [0])
...
for i in range (len(eora_sec)):
    if Gi.loc[eora_sec[i]].sum() == 0 :
        raise ValueError("sector {} in EORA has now concordance sector in SAM".format(eora_sec[i]))
...
a row is greater than 1, it means that some sectors in EORA 26 belong to 1 sector
in SAM together and should be aggregated and vice versa.
...
Eora = pd.read_excel(r'Databases\IO_KEN_2014_BasicPrice.xlsx',index_col = [0,1,2] , columns = [0])
#%%
